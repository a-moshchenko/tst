...
sports_list_id = {"American Football": 10}
coef_list = []
main_coefs_list = []
...
def check_main_coefficients(sport_id):
    ...
    try:
        ...
        if str(browser.current_url).split("=")[-1] != str(1):
            log_variable.error(
                f"Not current sport opened!ExpectedID{sport_id}CurrentID{(str(browser.current_url)[-2:])}")
...
def check_main_coefficients_live(sport_id):
    ...
    try:
        ...
        if str(browser.current_url).split("=")[-1] != str(1):
            log_variable.error(f"Not current sport opened!")
            browser.save_screenshot(f"ExpectedID{sport_id}CurrentID{(str(browser.current_url)[-2:])} ErrorNuxbet.png")
            return "Error"
        elements = browser.find_elements_by_xpath("//div[@class='numbersWrap']")
        param_elements = (browser.find_elements_by_xpath("//div[@class='hasParams numbersWrap']"))
